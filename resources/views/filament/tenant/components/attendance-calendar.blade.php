@php
    $logs = $record->attendanceLogs()
        ->whereMonth('punched_at', now()->month)
        ->whereYear('punched_at', now()->year)
        ->get()
    ...
    $firstDayOfMonth = now()->startOfMonth()->dayOfWeek; // 0 (Sun) to 6 (Sat)
@endphp

<div x-data="{ openLogModal: false, selectedDate: '', selectedShift: '', selectedLogs: [] }" class="mt-4 overflow-x-auto w-full relative">
    <div style="margin-bottom: 1rem; font-weight: bold; font-size: 1.125rem;">
        {{ now()->format('F Y') }}
    </div>
    ...
    <table style="width: 100%; border-collapse: separate; border-spacing: 0.5rem; table-layout: fixed;">
        ...
        <tbody>
            ...
            @for ($day = 1; $day <= $daysInMonth; $day++)
                @php
                    $dateString = now()->format('Y-m-') . str_pad($day, 2, '0', STR_PAD_LEFT);
